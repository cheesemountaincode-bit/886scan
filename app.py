...
class TPTier:
    enabled: bool
    pct_of_remaining: float   # 25.0 = 25% av kvarvarande qty
    mode: str                 # "MA" eller "ORIGINAL"
    ma_period: int = 0
    ma_tf: str = "15m"
    require_profit_first: bool = True  # endast relevant för MA
...
def first_ma_tp_signal(
    ...
    if str(pos_side).lower() == "long":
        tp_hit = sub[sub["close"] <= sub["ma"]]
    else:
        tp_hit = sub[sub["close"] >= sub["ma"]]

    if not tp_hit.empty:
        r = tp_hit.iloc[0]
        return True, float(r["close"]), int(r["ts"])
    return False, None, None
...
def compute_linear_fifo_pnl_and_events(
    ...
    if tp_tiers is None:
        tp_tiers = []
        if tp_enabled:
            tp_tiers.append(TPTier(
                enabled=True, pct_of_remaining=100.0, mode="MA",
                ma_period=tp_ma_period, ma_tf=tp_tf, require_profit_first=True
            ))
    ...
    for symbol, g in df.groupby("symbol", sort=False):
        ...
        for _, row in g.iterrows():
            ...
            price_fill = float(row.get("execPrice") or 0.0)
            ...
            if side.lower() == "buy":
                ...
                while qty_to_match > 0 and shorts:
                    lot = shorts[0]
                    take_total = min(qty_to_match, lot["qty"])
                    ...
                    for idx, tier in enumerate(tp_tiers, start=1):
                        ...
                        if tier.mode.upper() == "ORIGINAL":
                            ...
                            hit, tpp, tpts = first_ma_tp_signal(
                                ...
                                require_profit_first=bool(tier.require_profit_first)
                            )
                            if hit:
                    ...
                    for ev_ts, tag, ev_price, pct_rem in candidates:
                        if remaining <= 1e-12:
                            break
                        if last_ts_seen is not None and ev_ts < last_ts_seen:
                            continue
...
def run_combo(
    ...
    if multi_tp_preset is not None:
        ...
        tiers: List[TPTier] = []
        if p1 > 0:
            tiers.append(TPTier(True, float(p1), "MA", tp_period, tp_tf, profit_first_for_ma_tiers))
        if p2 > 0:
            tiers.append(TPTier(True, float(p2), "MA", tp_period, tp_tf, profit_first_for_ma_tiers))
        if p3 > 0:
            tiers.append(TPTier(True, float(p3), "MA", tp_period, tp_tf, profit_first_for_ma_tiers))
        tp_tiers = tiers
    ...
    return {
        ...
        "post_extend_days": post_extend_days,
        "multi_tp": multi_tp_preset if multi_tp_preset is not None else (100,0,0)
    }
...
def optimize_settings(
    ...
    tp_periods: List[int],
    tp_tfs: List[str],
    use_multi_tp_presets: bool,
    multi_tp_presets: List[Tuple[int,int,int]],
    profit_first_for_ma_tiers: bool
) -> pd.DataFrame:
    results = []
    ...
    for sp in stop_periods:
        for stf in stop_tfs:
            for tp in tp_periods:
                for ttf in tp_tfs:
                    if use_multi_tp_presets:
                        for preset in multi_tp_presets:
                            res = run_combo(
                                fpr, trades_df, sp, stf, tp, ttf,
                                allow_after_original, post_extend_days,
                                start_equity, allocation_pct,
                                multi_tp_preset=preset,
                                profit_first_for_ma_tiers=profit_first_for_ma_tiers
                            )
                            results.append(res)
                            cnt += 1
...
with st.sidebar:
    ...
    use_ma_tp = st.checkbox("Aktivera MA-TP (kräver vinstläge först)", value=True)
    ...
    if use_multi_tp:
        ...
        for i in range(1, 4):
            st.markdown(f"**TP{i}**")
            en = st.checkbox(f"Aktivera TP{i}", value=(i==1), key=f"tp{i}_en")
            pct = st.number_input(f"TP{i} – % av kvarvarande", min_value=1.0, max_value=100.0, value=33.0 if i<3 else 34.0, step=1.0, key=f"tp{i}_pct")
            ...
            if mode == "MA-korsning":
                ...
                tp_tiers_cfg.append(TPTier(en, float(pct), "MA", int(per), str(tfv), bool(prof)))
            else:
                tp_tiers_cfg.append(TPTier(en, float(pct), "ORIGINAL"))
...
def build_markers_df(events: pd.DataFrame) -> pd.DataFrame:
    ...
    def _map_event(x: str) -> str:
        ...
        if "_ORIG" in x: return x.replace("_ORIG","")
        if x == "ORIGINAL_ALL": return "ORIGINAL_ALL"
        return x
    m["event"] = m["exit_type"].apply(_map_event)
...
if markers_df.empty:
    ...
    if kdf.empty:
        ...
        if not PLOTLY_AVAILABLE:
            ...
            for tag in ["TP1","TP2","TP3","ORIGINAL_ALL"]:
                tps = subs[subs["event"]==tag]
                if not tps.empty:
                    fig.add_trace(go.Scatter(
...
with st.expander("Visa inställningar för optimering", expanded=st.session_state["opt_open"]):
    ...
    st.markdown("### Multi-TP i optimering (valfritt)")
    use_multi_tp_in_opt = st.checkbox("Använd Multi-TP-andelar i grid-sök", value=False,
                                      help="Testar samtidigt olika procentpaket för TP1/TP2/TP3.")
    multi_tp_presets = []
    if use_multi_tp_in_opt:
        st.caption("Fördefinierade paket (summa = 100%):")
        colp1, colp2, colp3 = st.columns(3)
        with colp1:
            p100 = st.checkbox("100 / 0 / 0", value=True)
            p5050 = st.checkbox("50 / 50 / 0", value=True)
        with colp2:
            p333334 = st.checkbox("33 / 33 / 34", value=True)
            p406020 = st.checkbox("40 / 60 / 0", value=False)
        with colp3:
            p307040 = st.checkbox("30 / 70 / 0", value=False)
            p257550 = st.checkbox("25 / 75 / 0", value=False)

        if p100:    multi_tp_presets.append((100,0,0))
        if p5050:   multi_tp_presets.append((50,50,0))
        if p333334: multi_tp_presets.append((33,33,34))
        if p406020: multi_tp_presets.append((40,60,0))
        if p307040: multi_tp_presets.append((30,70,0))
        if p257550: multi_tp_presets.append((25,75,0))

        prof_first = st.checkbox("Kräv vinstläge för MA-tiers", value=True,
                                 help="Gäller alla MA-baserade TP-tiers i optimeringen.")
    else:
        prof_first = True
    ...
    if start_equity <= 0 or alloc <= 0:
        ...
        with st.spinner("Optimerar kombinationer…"):
            opt_df = optimize_settings(
                ...
                use_multi_tp_presets=bool(use_multi_tp_in_opt),
                multi_tp_presets=list(multi_tp_presets),
                profit_first_for_ma_tiers=bool(prof_first),
            )
...
if opt_df is not None:
    if opt_df.empty:
        ...
        show_cols = ["final_eq","total_ret_pct","total_realized","avg_roi",
                     ...
                     "stop_period","stop_tf","tp_period","tp_tf","post_extend_days","multi_tp"]
        st.dataframe(opt_df[show_cols].head(15), use_container_width=True)
        best = opt_df.iloc[0]
        st.success(
            f"Bästa (just nu): Stop {int(best['stop_period'])}/{best['stop_tf']}, "
            f"TP {int(best['tp_period'])}/{best['tp_tf']}, Multi-TP {best['multi_tp']}, "
            f"Fönster {int(best['post_extend_days'])} dgr → "
